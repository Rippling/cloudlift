...
@_require_name
@_require_environment
@click.option('--local_tag', default=[], multiple=True,
              help='Use a locally built version instead of building image')
...
@click.option('--ssh', default=None, help='SSH agent socket or keys to expose to the docker build')
@click.option('--cache-from', multiple=True, help='Images to consider as cache sources')
def upload_to_ecr(name, environment, local_tag, additional_tags, build_arg, dockerfile, env_sample_file, ssh, cache_from):
    ServiceUpdater(name, environment=environment, env_sample_file=env_sample_file,
                   ...
                   ssh=ssh, cache_from=list(cache_from), local_tag=local_tag).upload_to_ecr(additional_tags)
